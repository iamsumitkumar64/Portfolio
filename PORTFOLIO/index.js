...
// Experience Tabs switching logic
const tabButtons = document.querySelectorAll('.tab-btn');
const tabPanels = document.querySelectorAll('.tab-panel');
const tabsIndicator = document.querySelector('.tabs-indicator');

if (tabButtons.length > 0 && tabPanels.length > 0 && tabsIndicator) {
    function updateTabIndicator(activeBtn) {
        const isMobile = window.innerWidth <= 768;
        if (isMobile) {
            tabsIndicator.style.left = `${activeBtn.offsetLeft}px`;
            tabsIndicator.style.width = `${activeBtn.offsetWidth}px`;
            tabsIndicator.style.top = 'auto';
            tabsIndicator.style.height = '2px';
        } else {
            tabsIndicator.style.top = `${activeBtn.offsetTop}px`;
            tabsIndicator.style.height = `${activeBtn.offsetHeight}px`;
            tabsIndicator.style.left = '-2px';
            tabsIndicator.style.width = '2px';
        }
    }
    ...
    if (activeBtn) {
        setTimeout(() => updateTabIndicator(activeBtn), 150);
    }
